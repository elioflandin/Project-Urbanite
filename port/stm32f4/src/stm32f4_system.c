...
bool stm32f4_system_gpio_read(GPIO_TypeDef *p_port, uint8_t pin)
{
    ...
    uint32_t result = p_port->IDR & mask;
...
void stm32f4_system_gpio_write(GPIO_TypeDef *p_port, uint8_t pin, bool value)
{
    ...
    if (value)
    {
        p_port->BSRR = mask;  // Write HIGH: set the bit in the GPIOx_BSRR lower half
        // p_port->ODR |= mask;  // Write in ODR to pass the unitary tests (not working)
    }
    else
    {
        p_port->BSRR = (mask << 16);  // Write LOW: set the bit in the GPIOx_BSRR upper half (reset)
        // p_port->ODR &= ~mask;  // Write in ODR to pass the unitary tests
    }
}

void stm32f4_system_gpio_toggle(GPIO_TypeDef *p_port, uint8_t pin)
{
    bool current_value = stm32f4_system_gpio_read(p_port, pin);  // Read the current state of the pin
    stm32f4_system_gpio_write(p_port, pin, !current_value);  // Write the opposite value to the pin
}
...
void port_system_sleep(void) {
  port_system_systick_suspend();  // 1. Suspend SysTick to avoid waking up every 1ms
  port_system_power_sleep();  // 2. Enter sleep mode to reduce power consumption
}


// decommente ligne 344 fait, 351 fait, 191 fait, 196 fait, 362 fait, 
// et fichier fsm_button.c et fsm_ultrasound.c à la fin (FAIT)
//  et fin de port_system.h, les 5 derniers prototypes (FAIT)
//  et dans EXTI15_10_IRQHandler à la ligne 45 de interr.c (FAIT) et dans TIM2_IRQHandler à la ligne 90 de interr.c (FAIT)
