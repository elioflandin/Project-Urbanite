 ...
 * 
 * @param p_port Port of the GPIO (CMSIS struct like, e.g., GPIOA)
 * @param pin Pin number (from 0 to 15)
 * @return true if the pin is HIGH, false if the pin is LOW
 */
bool stm32f4_system_gpio_read(GPIO_TypeDef *p_port, uint8_t pin);

/**
 * @brief Write a digital value to a GPIO pin atomically.
 * 
 * @param p_port Port of the GPIO (CMSIS struct like, e.g., GPIOA)
 * @param pin Pin number (from 0 to 15)
 * @param value Boolean value to write: true for HIGH, false for LOW
 */
void stm32f4_system_gpio_write(GPIO_TypeDef *p_port, uint8_t pin, bool value);

/**
 * @brief Toggle the digital value of a GPIO pin.
 * 
 * @param p_port Port of the GPIO (CMSIS struct like, e.g., GPIOA)
 * @param pin Pin number (from 0 to 15)
 */
